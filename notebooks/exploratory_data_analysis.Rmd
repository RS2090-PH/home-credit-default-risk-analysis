...
Based on our analysis of the loan amounts and applicant income, we observed a significant outlier in applicant income. This outlier has been removed in the second plot to allow for better visualization and comparison of the remaining amounts. Despite the removal of the outlier, a broad range of applicant income values remains, with the majority falling below the average values for the credit amounts provided and the cost of goods purchased. This suggests that the majority of applicants have relatively lower incomes in relation to the loan amounts they are seeking. 

Furthermore, when comparing the amount of credit with the price of goods purchased, the distributions of both variables appear strikingly similar. This is an obvious assumption, as loans are often requested specifically to cover the cost of goods or services, and the two variables are likely to be closely tied in most cases, reinforcing the connection between the two.

The loan annuity, however, is less informative in this plot as it is overshadowed by the distributions of the other amounts.
...
ggplot(temp_train, aes(x = age_group, fill = target)) +
  geom_bar(position = "fill", color = "steelblue", alpha = 0.6) +
  scale_fill_manual(values = c("No" = "steelblue", "Yes" = "pink")) +  # Colors for "No" and "Yes" defaults
  labs(
